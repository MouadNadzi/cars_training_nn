function pldistance(p1, p2, x, y) {
  ...
  class Vehicle {
    constructor(brain, visionAngle = 90) {
      ...
      this.index = 0;
      this.counter = 0;
      this.visionAngle = visionAngle; // Store vision angle


      // Get vision angle from slider
      //const visionAngle = visionAngleSlider.value();
      //this.rays = [];
      this.createRays(); // Call separate method to create rays


      
      // Calculate number of rays based on vision angle (one ray every 15 degrees)
      const rayCount = floor(visionAngle / 15);
      const angleStep = this.visionAngle / (rayCount - 1);

      const startAngle = -visionAngle / 2;
      const rayStep = 15;  // One ray every 15 degrees

      
      for (let a = startAngle; a < visionAngle/2; a += rayStep) {
      ...
      if (brain) {
        ...
        if (brain.input_nodes === this.rays.length) {
          this.brain = brain.copy();
      } else {
          console.log('Creating new brain due to input mismatch');
    ...
    createRays() {
        ...
        // Calculate angles for rays
        
            const rayCount = floor(this.visionAngle / 15);
            const angleStep = this.visionAngle / (rayCount - 1);
    ...
    // Add method to update vision angle
    updateVisionAngle(newAngle) {
      //this.visionAngle = newAngle;
      //this.createRays();
      // Update neural network input size if necessary
    
  if (this.visionAngle === newAngle) return; // Skip if no change
        
        this.visionAngle = newAngle;
        this.createRays();
        
        if (!this.brain || this.brain.input_nodes !== this.rays.length) {
    ...
    update() {
      if (!this.dead && !this.finished) {
        ...
        // Update ray positions and rotations
        const heading = this.vel.heading();
        for (let ray of this.rays) {
            // Update ray position to vehicle position
            ray.pos = this.pos.copy();
            // Update ray direction based on vehicle heading
            ray.rotate(heading);
        }
